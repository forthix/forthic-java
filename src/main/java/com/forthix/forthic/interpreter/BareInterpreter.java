...
public class BareInterpreter {
  ...
  protected void handleToken(Token token) throws Exception {
    switch (token.getType()) {
      ...
      case END_DEF:
        ...
        break;
      case START_MEMO:
        handleStartMemoToken(token);
        break;
      case WORD:
        handleWordToken(token);
        break;
      case DOT_SYMBOL:
        handleDotSymbolToken(token);
        break;
      case EOS:
        if (isCompiling) {
        ...
        break;
      default:
        throw new UnknownTokenError(getTopInputString(), token.getString(), token.getLocation());
    }
  }
  ...
  protected void handleWordToken(Token token) throws Exception {
    ForthicWord word = findWord(token.getString());
    if (word == null) {
  ...
  protected ForthicWord findLiteralWord(String name) {
  ...
  protected ForthicWord findWord(String name) {
    // 1. Check module stack (dictionary words + variables)
    ForthicWord result = curModule().findWord(name);
    if (result == null) {
  ...
  protected static class StartModuleWord extends ForthicWord {
  ...
  protected static class EndModuleWord extends ForthicWord {
  ...
  protected static class EndArrayWord extends ForthicWord {
    public EndArrayWord() {
      super("]");
    }
